  ...
  useTicket(id) {
    ...
    fetch("http://192.168.1.100:8000/api/station/useticket", {
      ...
      .then(res => {
        ...
        console.log(res);
        if (res.ok) {
          Alert.alert("Vé hợp lệ","Xin mời quý khách");
          
          // this.storeToken(res.body.success.token);
          // this.props.navigation.navigate("Home");
        } else {
          Alert.alert("Vé không hợp lệ", "Vé quý khách sử dụng đã hết hạn hoặc bị lỗi, liên hệ với trung tâm chăm sóc khách hàng để được hỗ trợ");
        }
      })
      .catch(error => {
        console.error(error);
